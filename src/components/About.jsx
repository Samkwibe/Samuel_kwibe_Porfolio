  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-br from-gray-900 via-blue-900/20 to-purple-900/10 overflow-hidden relative">
      ...
      <main className="flex-1 relative z-10">
        <div className="w-full px-4 sm:px-6 md:px-8 lg:px-12 xl:px-16 py-16 md:py-20">
          {/* Page Header */}
          <motion.header 
            ref={headerRef}
          ...
          >
            <motion.div 
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-cyan-500/10 border border-cyan-400/30 mb-6"
            ...
            <p className="text-2xl md:text-3xl lg:text-4xl opacity-90 max-w-6xl mx-auto leading-relaxed font-light">
          ...
          <motion.section 
            ...
            transition={{ duration: 0.8 }}
            className="grid lg:grid-cols-3 gap-10 md:gap-12 mb-20 md:mb-24 w-full"
          >
            {/* Profile Card */}
            <motion.div 
            ...
            >
              <div className="card p-8 md:p-10 bg-gradient-to-br from-gray-900/80 to-blue-900/20 rounded-3xl border border-white/10 shadow-2xl backdrop-blur-sm">
                <div className="text-center">
                  <motion.div 
                    className="relative mx-auto w-32 h-32 md:w-40 md:h-40 rounded-full p-1 bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-600"
                    whileHover={{ scale: 1.05 }}
                  >
                    {imageError ? (
                      <div className="w-full h-full rounded-full bg-gray-800 flex items-center justify-center text-white text-3xl md:text-4xl font-bold">
                        SK
                      </div>
                    ) : (
                      <img 
                  ...
                  <h2 className="text-2xl md:text-3xl font-bold mt-6">{profile.name}</h2>
                  <p className="opacity-70 mt-2 text-base md:text-lg">{profile.title}</p>
            ...
            >
              <div className="card p-8 md:p-10 bg-gradient-to-br from-gray-900/80 to-purple-900/20 rounded-3xl border border-white/10 shadow-2xl backdrop-blur-sm h-full">
                ...
                <div className="grid gap-4 mb-8">
                  {highlights.map((highlight, index) => {
                    ...
                    return (
                      <motion.div 
                        key={index} 
                        className="flex items-center gap-4 p-4 md:p-6 rounded-xl bg-white/5 hover:bg-white/10 transition-all duration-300 group"
                        initial={{ opacity: 0, y: 20 }}
                        animate={profileInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
                        transition={{ duration: 0.5, delay: 0.6 + index * 0.1 }}
                        whileHover={{ scale: 1.02, x: 5 }}
                      >
                        <Icon className={`w-6 h-6 md:w-8 md:h-8 text-cyan-400 group-hover:scale-110 transition-transform bg-gradient-to-r ${highlight.gradient} bg-clip-text text-transparent`} />
                        <span className="opacity-90 text-base md:text-lg">{highlight.text}</span>
                      </motion.div>
                    )
                  })}
                </div>
          ...
          >
            ...
            <motion.div 
              className="card p-10 md:p-12 bg-gradient-to-br from-gray-900/80 to-blue-900/20 rounded-3xl border border-white/10 shadow-2xl backdrop-blur-sm w-full"
              initial={{ opacity: 0 }}
